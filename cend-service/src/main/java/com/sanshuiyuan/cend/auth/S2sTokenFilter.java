...
/**
 * Service-to-service token 过滤器：Bearer S2S token 即授予 ROLE_S2S 认证。
 * 仅当请求尚未认证时生效（优先级低于 H5 JWT）。
 * matching-service 调用 /internal/wxmsg/** 时使用，与 matching 侧共享 ${s2s.token}。
 */
public class S2sTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (token != null && token.equals(s2sToken)) {
            ...
            // 普通已认证用户（NO_AUTHORITIES）——后者不得访问 /internal/** 内部端点。
