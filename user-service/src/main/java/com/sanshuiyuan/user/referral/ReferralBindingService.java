...
/**
 * 小程序关系链绑定服务（008b / 014，自 h5-service 忠实移植）。微信小程序登录时仅定位/创建本人
 * （见 {@code WxLoginUseCase}，本服务不参与登录）；L1/L2 关系链改由用户在落地页 <b>显式确认</b>后经
 * {@link #confirmBinding(Long, String)} 一次性写入。
 *
 * <p><b>合规铁律</b>：
 * <ul>
 *   <li>关系链<b>仅首次绑定写入</b>；已绑定用户再次确认幂等返回，绝不触碰其 {@code inviter_id}/{@code grand_inviter_id}；</li>
 *   <li>ref_id 解码失败按<b>自然流量</b>处理，绝不抛错、不阻断；</li>
 *   <li>L2（{@code grandInviterId}）仅取「邀请人的 inviter_id」一次性快照，<b>不向上递归</b>（L3+ 物理隔离）。</li>
 * </ul>
 *
 * <p><b>登录不绑关系链</b>：本服务不参与微信登录链路；登录仅由 {@code WxLoginUseCase} 定位/创建本人，
 * 绑定只能由用户显式确认经 {@link #confirmBinding} 触发。
 */
@Service
public class ReferralBindingService {
    ...
    public ReferralBindingService(UserRepository userRepo, RefIdCodec refIdCodec) {
     ...
     * 命中则不绑定、不报错。
     *
     * @param currentUserId 当前已登录用户 user_id（由 controller 自 {@code @AuthenticationPrincipal} 取得）
     * @param refId         推广 ref_id（推广者 user_id 的 HMAC 签名形式）
     * @return {@code true} 表示本次新建立了绑定；{@code false} 表示已绑定（幂等）或降级未绑定。
     */
    @Transactional
    public boolean confirmBinding(Long currentUserId, String refId) {
    ...
    private Long resolveInviterId(String refId) {
        ...
        try {
            ...
            // 伪造/篡改/格式非法：按自然流量处理，仅记审计日志，不暴露细节、不阻断。
            log.info("ref_id 解码失败，按自然流量处理");
            return null;
        }
    }
}
