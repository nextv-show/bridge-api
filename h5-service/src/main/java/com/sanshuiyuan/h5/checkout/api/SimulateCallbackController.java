...
public class SimulateCallbackController {
    ...
    public SimulateCallbackController(H5OrderRepository orderRepo, PaymentInboxRepository inboxRepo) {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> simulate(@RequestBody Map<String, String> req) {
        ...
        if (orderNo == null) {
            return ResponseEntity.badRequest()
                    .body(ApiResponse.error(com.sanshuiyuan.h5.common.ErrorCode.VALIDATION_FAILED, "orderNo required"));
        }

        var orderOpt = orderRepo.findByOrderNo(orderNo);
        if (orderOpt.isEmpty()) {
            return ResponseEntity.status(404)
                    .body(ApiResponse.error(com.sanshuiyuan.h5.common.ErrorCode.ORDER_NOT_FOUND));
        }

        H5Order order = orderOpt.get();
        if (order.getStatus() != OrderStatus.PENDING_PAY) {
            return ResponseEntity.status(409)
                    .body(ApiResponse.error(com.sanshuiyuan.h5.common.ErrorCode.ORDER_STATUS_CONFLICT));
        }
