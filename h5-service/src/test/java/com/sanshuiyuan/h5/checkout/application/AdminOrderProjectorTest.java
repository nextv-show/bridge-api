 ...
 * device_assets 是 admin-service 拥有的 h5_db 真表，h5-service 测试容器无该表，故不走集成测试；
...
class AdminOrderProjectorTest {
    ...
    private H5Order paidOrder() {
        H5Order o = H5Order.create("H5TEST001", "openid-x", "home-std", "BR-H1", 460000L, "wechat");
    ...
    void pending_pay_order_does_not_project_device_asset() {
        H5Order pending = H5Order.create("H5TEST002", "openid-y", "home-std", "BR-H1", 460000L, "wechat");
